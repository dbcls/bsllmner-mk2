...
- **Stage 2b: text2term fallback (`text2term_sec`).** For OWL files only, `text2term.map_terms(..., target_ontology=<acronym>, use_cache=True, cache_folder=BSLLMNER2_TEXT2TERM_CACHE_DIR)` is used as a similarity-based fallback. The acronym is `{ontology_file_stem}_{filter_hash}`, matching the `filter_hash` used by the word-combination index so both caches invalidate together. When the text2term cache build fails (e.g. read-only cache dir), the call falls back to `target_ontology=<owl_path>` with `use_cache=False` and the run continues with a warning
