...
from bsllmner2.models import CliExtractArgs, ErrorLog, ExtractEntry, ExtractResult, PerformanceSummary, StageTimings
...
async def run_cli_extract_async() -> None:
    ...
    extract_result_file: Path | None = None
    ...
    try:
        async with run_with_lifecycle() as run_state:
            ...
            async def process_extract_batch(
            ...
            ) -> tuple[list[ExtractEntry], list[ChatResponse], float, int, list[ErrorLog]]:
                with stage_timer("ner") as t_ner:
                    batch_outputs, batch_chat_responses, batch_errors, batch_errors_log = await ner(
                        backend,
                        batch_info.entries,
                        prompt,
                        format_,
                ...
                return batch_outputs, batch_chat_responses, t_ner.elapsed_sec, batch_errors, batch_errors_log

            def on_extract_batch_complete(
                batch_idx: int,
                batch_result: tuple[list[ExtractEntry], list[ChatResponse], float, int, list[ErrorLog]],
            ) -> None:
                batch_outputs, batch_chat_responses, ner_sec, batch_errors, batch_errors_log = batch_result
